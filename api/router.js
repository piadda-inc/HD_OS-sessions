...
const { handleImplementationCommand } = require('./implementation.js');
...
const COMMAND_HANDLERS = {
    ...
    'implementation': handleImplementationCommand,
...
const ROOT_HELP_LINES = [
    ...
    'tasks          - idx, start',
    'implementation - begin, status, extend, end, locks, ownership',
...
Features: branch_enforcement, task_detection, auto_ultrathink, icon_style, warn_85, warn_90`,
  ...
  start @<task>   - Start working on a task`,
    ...
    "implementation": `Available implementation commands:
  ...
  ownership <file>     - Check who owns a specific file`,
};
...
function routeCommand(command, args, jsonOutput = false, fromSlash = false) {
    ...
    if (fromSlash) {
        try {
            ...
            if (['config', 'state', 'tasks', 'implementation', 'uninstall'].includes(command)) {
